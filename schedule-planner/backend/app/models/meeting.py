...
def init_meeting_db() -> None:
    ...
    try:
        ...
        # 创建索引
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_meeting_creator ON meetings(creator_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_participant_meeting ON meeting_participants(meeting_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_participant_user ON meeting_participants(user_id)')
...
def get_meetings_by_user_id(user_id: int, start_date: Optional[datetime.date] = None, end_date: Optional[datetime.date] = None) -> List[Dict[str, Any]]:
    """根据用户ID获取其参加的所有会议。
...
def create_meeting_in_db(meeting_data: Dict[str, Any], creator_id: int, participant_ids: List[int], key_participant_ids: List[int]) -> Tuple[Optional[Dict[str, Any]], str]:
    """在数据库中创建新会议及其参与者。
